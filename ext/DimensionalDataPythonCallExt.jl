...
function _attrs(x)
    ...
    valid = if contents isa Union{AbstractDict, NamedTuple}
        ...
        @warn "$(nameof(typeof(x))) metadata must be a dictionary or NamedTuple type to pass to xarray, but the passed metadata is a $(typeof(contents)). " *
...
function PythonCall.Py(data::DD.AbstractDimArray; copy=false, xarray=nothing)
    if !_use_xarray(xarray)
        return PythonCall.pyjl(_maybecopy(data, copy))
    end
    ...
    # Julia and Python. `np.array()` is used instead of `.to_numpy()` because
    # `Py(values)` is already an ndarray when `values` is a `PyArray`.
    return xr.DataArray(np.array(Py(values); copy).T;
